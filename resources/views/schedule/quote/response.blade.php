  ...
  <form method="POST" action="{{url('usuario/citas/agendar/cita')}}" autocomplete="off" enctype="multipart/form-data">
    {{csrf_field() }}
     <div class="form-row">
      ...
      <div class="form-group col-md-6">
        <label for="quote_date"><strong>Consultar fecha {{$day}}:</strong></label>
        <input type="date" class="form-control" id="quote_date" name="quote_date" placeholder="" required="" value="{{$day}}">
        <!-- span>Fechas disponibles del 2 al 6 de Agosto.</span-->
        <span class="alert-danger">{{$errors->first('quote_date')}}</span>
          
          @if($quotes =='Sin disponibilidad')
          <table class="table">
              <tr>
                  <td><span class="text-danger text-b">Horarios no disponibles</span></td>
              </tr>
          </table>
          @else
          <table id='horario' class="table">
            ...
            <tbody>
                ...
                <?php    
                if (in_array($quote->date_hour, $horarios)) {
                }else{
                    array_push($horarios, $quote->date_hour);
                ?>
                <tr>
                  <td >
                    <div class="ml-3">
                      <input id="item{{$quote->id}}" class="form-check-input " type="radio" name="quote_date_id" value="{{$quote->id}}"><label for="item{{$quote->id}}">{{$quote->date_hour}} hrs</label>
                    </div>
                  </td>
                </tr>
                <?php }?> 
